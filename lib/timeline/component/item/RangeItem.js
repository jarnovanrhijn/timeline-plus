 ...
 */
  ...
  _updateDomComponentsSizes(sizes) {
    ...
    fastdom.measure(() => {
      ...
      const domDontent = this.dom.content;

      // Bunch together the writes.
      fastdom.mutate(() => {
        content.width = sizes.content.width;
        domDontent.style.maxWidth = '';
      });
    });
    ...
    fastdom.measure(() => {
      ...
      fastdom.mutate(() => {
        switch (align) {
          ...
          case 'right':
            if (this.options.rtl) {
              content.style.right = `${Math.max((boxWidth - contentWidth), 0)}px`;
            } else {
              content.style.transform = `translateX(${Math.max((boxWidth - contentWidth), 0)}px)`;
            }
            break;

          case 'center':
            if (this.options.rtl) {
              content.style.right = `${Math.max((boxWidth - contentWidth) / 2, 0)}px`;
            } else {
              content.style.transform = `translateX(${Math.max((boxWidth - contentWidth) / 2, 0)}px)`;
            }
            break;
          ...
          default: // 'auto'
            ...
            if (this.options.rtl) {
              content.style.right = `${contentStartPosition}px`;
            } else {
              content.style.transform = `translateX(${contentStartPosition}px)`;
            }
        }
      });
    });
  ...
  repositionY() {
    ...
    fastdom.measure(() => {
      ...
      fastdom.mutate(() => {
        if (orientation == 'top') {
          boxStyle.top = `${this.top}px`;
        }
        else {
          boxStyle.top = `${this.parent.height - this.top - this.height}px`;
        }
      });
    });
  }
