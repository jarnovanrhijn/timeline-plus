 ...
 */
  ...
  repositionY(margin) {  // eslint-disable-line no-unused-vars
    ...
    // Bunch together the reads.
    fastdom.measure(() => {
      const box = this.dom.box;
      // TODO: instead of calculating the top position of the subgroups here for every BackgroundItem, calculate the top of the subgroup once in Itemset
      const itemSubgroup = this.data.subgroup;
      const parentSubGroup = this.parent.subgroups;
      const parent = this.parent;
      // Bunch together the writes.
      fastdom.mutate(() => {// special positioning for subgroups
        if (this.data.subgroup !== undefined) {

          box.style.height = `${parentSubGroup[itemSubgroup].height}px`;

          if (orientation == 'top') {
            box.style.top = `${parent.top + parentSubGroup[itemSubgroup].top}px`;
          } else {
            box.style.top = `${parent.top + parent.height - parentSubGroup[itemSubgroup].top - parentSubGroup[itemSubgroup].height}px`;
          }
          box.style.bottom = '';
        }
        // and in the case of no subgroups:
          ...
          if (parent instanceof BackgroundGroup) {
            // if the item is not in a group:
            height = Math.max(parent.height,
                parent.itemSet.body.domProps.center.height,
                parent.itemSet.body.domProps.centerContainer.height);
            box.style.bottom = orientation == 'bottom' ? '0' : '';
            box.style.top = orientation == 'top' ? '0' : '';
          }
          else {
            height = parent.height;
            // same alignment for items when orientation is top or bottom
            box.style.top = `${this.parent.top}px`;
            box.style.bottom = '';
          }
        }
